...
export default async function Navbar() {
  ...
  return (
    <>
      <nav className="sticky top-0 left-0 z-[2] px-8 py-4 bg-background flex justify-between items-center border-b border-solid border-black/20">
        ...
        <Link href="/" className="flex items-center">
          <span className={`${righteous.className} text-primary text-2xl`}>TOOLSHOP</span>
          <svg width="50px" height="50px" viewBox="0 0 213 213">
      ...
      <div className="flex justify-between items-center text-sm px-8 w-full bg-background border-b border-solid border-black/20">
        <div className="flex items-center gap-2">
          {categories?.map((item) => {
            return (
              <div key={item._id} role="listitem" className="dropdown dropdown-hover">
                <div tabIndex={0} className="group py-4 cursor-pointer">
                  <Link
                    href={`/products?category=${item.name}`}
                    className="px-2 align-middle transition-colors group-hover:text-accent"
                  >
                    {item.name}
                  </Link>
                  <svg
                    className="inline-block stroke-primary align-middle transition-transform group-hover:rotate-180"
                    width="20"
                    height="20"
                    viewBox="0 0 24 24"
                    fill="none"
        ...
        <Link href="/frequently-questions">
          <span className="align-middle">راهنمای خرید</span>
          <ShoppingHelp />
        </Link>
      </div>
    </>
