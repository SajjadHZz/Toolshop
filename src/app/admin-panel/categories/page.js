...
export default function AddProducts() {
  ...
  const ENDPOINT = "https://storage.iran.liara.space/Categories";
  ...
  return (
    <div className="bg-background flex-1 w-2/3">
      <h3 className="text-3xl font-bold m-4">افزودن دسته‌بندی</h3>
      <hr />
      <form onSubmit={addProductHandler}>
        ...
        <div className="flex-1">
          <label className="input input-bordered flex items-center gap-2">
            <button
              type="button"
              onClick={() => setSubs((prev) => [...prev, subsInput])}
            ...
            <input
              ...
              type="text"
              className="grow"
              placeholder="زیرشاخه های دسته بندی..."
            />
          </label>
          <ul className="">
            {subs.map((item, index) => {
